...
class WebsiteSale(http.Controller):
    ...
    def _get_shop_payment_values(self, order, **kwargs):
        ...
        if order:
            values['acq_extra_fees'] = acquirers.get_acquirer_extra_fees(order.amount_total, order.currency_id,
    ...
    def _get_products_recently_viewed(self):
        ...
        if visitor:
            ...
            if products_ids:
                viewed_products = request.env['product.product'].with_context(display_default_code=False).browse(
                    products_ids)
    ...
    def get_dynamic_snippet_filters(self):
        domain = expression.AND([
            ...
            ['|', ('filter_id.model_id', '=', 'product.product'),
             ('action_server_id.model_id.model', '=', 'product.product')]
        ])
